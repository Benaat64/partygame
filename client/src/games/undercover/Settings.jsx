...
const categoryLabels = { animals: 'Animaux', food: 'Nourriture', anime: 'Anime', 'pop-culture': 'Comics & Jeux vidéo' };

export default function Settings({ room, isHost, disabled, onSettings, onStart }) {
  const [search,setSearch]=useState('');
  const { settings, gameOptions, players } = room;
  const canStart = gameOptions.pairCount > 0 && players.length >= 3 && players.length - settings.undercoverCount > settings.undercoverCount;
  ...
  return (
    <div>
      <fieldset className="min-w-0 space-y-4 disabled:opacity-65" disabled={disabled || !isHost}>
        ...
        {(settings.categories.includes('anime')||settings.categories.includes('pop-culture'))&&<div className="space-y-3"><p className="text-sm font-medium">Univers connus des joueurs</p><Input aria-label="Rechercher un univers" placeholder="Rechercher un univers…" value={search} onChange={e=>setSearch(e.target.value)} /><div className="flex gap-3"><Button type="button" variant="ghost" onClick={()=>update({universes:gameOptions.universes})}>Tout sélectionner</Button><Button type="button" variant="ghost" onClick={()=>update({universes:[]})}>Tout retirer</Button></div><div className="flex flex-wrap gap-2">{gameOptions.universes.filter(u=>u.includes(search.toLowerCase().replaceAll(' ','-'))).map(u=><label key={u} className="flex min-h-11 items-center gap-2 rounded-xl border px-3 text-sm"><input type="checkbox" className="accent-primary" checked={settings.universes.includes(u)} onChange={e=>update({universes:e.target.checked?[...settings.universes,u]:settings.universes.filter(v=>v!==u)})} /><span className="capitalize">{u.replaceAll('-',' ')}</span></label>)}</div><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={settings.crossovers} onChange={e=>update({crossovers:e.target.checked})} />Autoriser les crossovers entre univers sélectionnés</label><p className="text-xs text-muted-foreground">Les objets et métiers courants restent possibles avec Anime seul. Toutes les références spécialisées d’une paire doivent être autorisées.</p></div>}
        <p role="status" className="text-sm font-semibold text-primary">{gameOptions.pairCount} paires disponibles</p>
      </fieldset>
