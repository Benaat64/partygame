...
import { Gavel, Wallet, ArrowLeftRight } from 'lucide-react';
...
function Pitch({player,team,editable=false,disabled,onPlace}) {
  return <section className="min-w-0 rounded-3xl border border-border bg-card p-4">
    ...
    <div className="mb-2 flex justify-end rounded-t-2xl border border-white/10 bg-zinc-900/60 p-3">
      <div className="flex max-w-full items-center gap-3 rounded-xl border border-primary/30 bg-background/80 px-3 py-2" aria-label="Entraîneur sur la ligne de touche">
        <PlayerPhoto key={team.slots.Coach?.id ?? "empty-coach"} player={team.slots.Coach ?? {position:"Coach"}} className="h-14 w-12" />
        <div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-widest text-primary">Coach · Ligne de touche</p><p className="break-words text-xs font-semibold">{team.slots.Coach?.name ?? 'À recruter'}</p>{team.slots.Coach&&<p className="text-[10px] text-muted-foreground">{team.slots.Coach.price} €</p>}</div>
      </div>
    </div>
    <div className="relative h-[480px] overflow-hidden rounded-2xl border border-emerald-200/30 bg-gradient-to-b from-emerald-900 to-emerald-950">
      ...
      {positions.map(([slot,position])=>{const card=team.slots[slot];return <div key={slot} className={`absolute ${position} flex w-[42%] max-w-40 -translate-x-1/2 -translate-y-1/2 flex-col items-center rounded-xl border p-2 text-center shadow-lg ${card?'border-primary/50 bg-background/95':'border-white/20 bg-black/20'}`}>
        <span className="text-[10px] font-bold tracking-widest text-emerald-200">{slot}</span><PlayerPhoto key={card?.id ?? slot} player={card} className="my-1 h-10 w-12" /><span className="text-xs font-semibold [overflow-wrap:anywhere]">{card?.name ?? 'À recruter'}</span>{card&&<span className="mt-1 text-[10px] text-muted-foreground">{card.position} · {card.price} €</span>}
      </div>})}
    </div>
    {editable&&<div className="mt-4 space-y-3"><p className="text-sm text-muted-foreground">Place tes recrues librement. Une place occupée échange les deux recrues.</p>{Object.entries(team.slots).map(([slot,card])=><label key={card.id} className="flex items-center justify-between gap-2 text-xs"><span className="min-w-0 break-words">{card.name}</span><select aria-label={`Placement de ${card.name}`} className="rounded-lg border bg-background p-2 text-foreground" value={slot} disabled={disabled} onChange={e=>onPlace(card.id,e.target.value)}>{['GB','DC','MC','ATT','Joker','Coach'].map(target=><option key={target} value={target}>{target==='Coach'?'Touche':target}</option>)}</select></label>)}</div>}
  </section>;
}
function Bidding({room,playerId,disabled,onAction}) {
  ...
  return <section className="rounded-3xl border border-primary/30 bg-card p-4 sm:p-6">
    ...
    <div className="my-5 text-center"><PlayerPhoto key={g.current.id} player={g.current} className="mx-auto mb-3 h-40 w-40 sm:h-48 sm:w-48" /><p className="text-sm text-muted-foreground">{g.current.position}</p><h2 className="text-3xl font-black">{g.current.name}</h2><p className="mt-3 text-xl text-primary">{g.price ? `${g.price} € · ${room.players.find(p=>p.id===g.leaderId)?.nickname}`:'Mise de départ : 1 €'}</p></div>
...
export default function FootballGame({session,disabled,onAction,onReset,onLeave}) {
  ...
  const placement=player=>({editable:player.id===playerId,disabled:disabled||room.paused,onPlace:(recruitId,slot)=>onAction('place',{playerId:recruitId,slot,gameSessionId:room.gameSessionId})});
  return <div className="py-6"><h1 className="text-3xl font-black">{finished?'Deux équipes. À vous de comparer !':'Le mercato est ouvert.'}</h1><p className="mt-2 text-muted-foreground">{room.settings.budget} € de départ · GB, DC, MC, ATT, Joker et Coach · Ouverture alternée A / B</p>
    {finished ? <div className="my-6 grid gap-5 lg:grid-cols-2">{ordered.map(player=><Pitch {...placement(player)} key={player.id} player={player} team={g.teams[player.id]} />)}</div> : <div className="my-6 grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)_minmax(0,1fr)]">
      <div className="hidden lg:block"><Pitch {...placement(ordered[0])} player={ordered[0]} team={g.teams[ordered[0].id]} /></div>
      ...
      <div className="hidden lg:block"><Pitch {...placement(ordered[1])} player={ordered[1]} team={g.teams[ordered[1].id]} /></div>
      <div className="lg:hidden">
        ...
        <div id="mobile-pitch"><Pitch {...placement(visible)} player={visible} team={g.teams[visible.id]} /></div>
      </div>
    </div>}
    ...
    <p className="my-4 text-xs text-muted-foreground">Six recrues, placement libre : cinq sur le terrain, une sur la touche. Le poste indiqué est informatif. Réorganise ton équipe pendant les enchères ou à la fin. Tant que les deux équipes sont incomplètes, chaque recrutement est confirmé. Dès qu’une équipe est complète, l’autre est complétée automatiquement à 1 € par recrue. Une coupure réserve la place 60 secondes.</p>
    <p className="mb-4 text-xs text-muted-foreground">Photos et données : <a className="underline hover:text-foreground" href="https://www.thesportsdb.com" target="_blank" rel="noreferrer">TheSportsDB</a>.</p>
