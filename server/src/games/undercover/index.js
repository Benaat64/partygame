...
function validateSettings(input) {
  if (
    !input ||
    !Number.isInteger(input.undercoverCount) ||
    input.undercoverCount < 1 ||
  ...
  if (
    ...
    (input.universes !== undefined &&
      (!Array.isArray(input.universes) ||
        input.universes.some((u) => !universes.includes(u)))) ||
    (input.crossovers !== undefined && typeof input.crossovers !== 'boolean')
  ) {
    throw new Error('Sélectionnez des thèmes et univers valides.');
  }
  const phaseSeconds =
    input.phaseSeconds === undefined ? 30 : input.phaseSeconds;
  const matchCount = input.matchCount ?? 3;
  if (
    ![null, 10, 20, 30, 40, 60].includes(phaseSeconds) ||
    !Number.isInteger(matchCount) ||
    matchCount < 1 ||
    matchCount > 5
  ) {
    throw new Error(
      'Choisissez 10, 20, 30, 40 ou 60 secondes ou Illimité, 1 à 5 manches.',
    );
  }
  const settings = {
    phaseSeconds,
...
export const undercover = {
  privateState(state, playerId) {
    const card = state?.assignments.get(playerId);
  ...
  options: (settings) => ({
    ...
    universes: [...universes],
    pairCount: settings ? matchingPairs(settings).length : 0,
  }),
  validateSettings,
  next(state, players) {
    if (
      state.phase !== 'finished' ||
      state.match >= state.settings.matchCount
    ) {
  ...
  start(players, input) {
    ...
    if (players.length < 3)
      throw new Error('Il faut au moins 3 joueurs pour lancer Undercover.');
    ...
    if (!pairs.length)
      throw new Error(
        'Aucune paire disponible : ajoutez un thème ou un univers.',
      );
    ...
    const assignments = new Map(
      shuffled.map((player, index) => {
        if (index < intruders)
          return [player.id, { role: 'undercover', word: pair.word2 }];
