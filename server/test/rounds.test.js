...
import { initializeRoundState, act, advance, publicState } from '../src/games/undercover/rounds.js';
import { createRoomStore } from '../src/rooms/store.js';
function setup(maxTurns = 5) {
  return initializeRoundState(new Map(['a', 'b', 'c', 'd', 'e'].map(id => [id, { role: id === 'a' ? 'undercover' : 'civil', word: id === 'a' ? 'Loup' : 'Chien' }])), { phaseSeconds: 10, maxTurns, matchCount: 1 }, 0);
}
function clues(s) { for (const id of s.clueOrder) act(s, id, 'clue', { text: 'Indice', turn: s.turn }, 1); }
function vote(s, id, targetId) { act(s, id, 'vote', { targetId, turn: s.turn }, 2); }
test('indices, validation, votes uniques, victoire civile et secrets', () => {
  ...
  assert.throws(() => act(s, 'a', 'clue', { text: 'x'.repeat(121), turn: 1 }, 1));
  const first = s.clueOrder[0];
  act(s, first, 'clue', { text: 'Forêt', turn: 1 }, 1);
  assert.throws(() => act(s, first, 'clue', { text: 'Bis', turn: 1 }, 1));
  for (const id of s.clueOrder.slice(1)) act(s, id, 'clue', { text: 'Poils', turn: 1 }, 1);
...
test('élimination, actions interdites et victoire par parité', () => {
  ...
  clues(s); assert.throws(() => vote(s, 'a', 'b'));
  for (const id of ['a','d','e']) vote(s, id, 'c');
  vote(s, 'c', 'd'); clues(s);
  vote(s, 'a', 'd'); vote(s, 'e', 'd'); vote(s, 'd', 'e');
  assert.equal(s.winner, 'undercover');
});
test('timeouts, égalité, abstentions et limite de manches', () => {
  const s = setup(2);
  assert.equal(advance(s, 9999), false);
  advance(s, 10000);
  assert.throws(() => act(s, 'a', 'vote', { targetId: 'b', turn: 1 }, 20000));
  for (let now = 20000; now <= 60000; now += 10000) advance(s, now);
  assert.equal(s.results[0].eliminated, null);
  for (let now = 70000; now <= 110000; now += 10000) advance(s, now);
  act(s, 'a', 'vote', { targetId: 'b', turn: 2 }, 110001);
  act(s, 'b', 'vote', { targetId: 'a', turn: 2 }, 110001);
  advance(s, 120000);
  assert.equal(s.results[1].eliminated, null);
  assert.equal(s.winner, 'undercover');
  assert.equal(advance(s, 999999), false);
});
test('création, horloge serveur, anciennes parties et rejouer', () => {
  const store = createRoomStore();
  assert.throws(() => store.create('bad', 'Alice', { phaseSeconds: 1 }));
  const { room } = store.create('a', 'Alice', { phaseSeconds: 10, maxTurns: 1 });
  store.join('b','Bob',room.code); store.join('c','Charlie',room.code);
  const started = store.start('a');
  assert.equal(started.room.settings.maxTurns, 1);
  assert.throws(() => store.action('a','clue',{ gameSessionId:'old',turn:1,text:'Indice' }));
  const deadline = started.room.game.deadline;
  store.tick(deadline); store.tick(deadline + 10000);
  assert.equal(store.tick(deadline + 20000)[0].game.phase, 'vote');
  assert.equal(store.tick(deadline + 30000)[0].game.winner, 'undercover');
  store.reset('a');
...
test('manches successives : scores cumulés une fois, cartes privées et remise en jeu', () => {
  const store = createRoomStore();
  const { room } = store.create('a', 'Alice', { phaseSeconds: 10, matchCount: 2, maxTurns: 1 });
  store.join('b', 'Bob', room.code); store.join('c', 'Charlie', room.code);
  const first = store.start('a');
  assert.throws(() => store.next('a'));
  const deadline = first.room.game.deadline;
  store.tick(deadline); store.tick(deadline + 10000); store.tick(deadline + 20000);
  const end = store.tick(deadline + 30000)[0];
  assert.equal(end.game.winner, 'undercover');
  assert.equal(end.game.seriesFinished, false);
  assert.equal(Object.values(end.game.scores).reduce((a,b) => a+b,0), 3);
  ...
  assert.throws(() => store.action('a','clue', { gameSessionId: first.room.gameSessionId, turn: 1, text: 'Ancien' }));
  store.tick(second.room.game.deadline); store.tick(second.room.game.deadline + 10000); store.tick(second.room.game.deadline + 20000);
  const final = store.tick(second.room.game.deadline + 30000)[0];
  assert.equal(final.game.seriesFinished, true);
  assert.equal(final.game.history.length, 2);
  assert.equal(Object.values(final.game.scores).reduce((a,b) => a+b,0), 6);
  assert.throws(() => store.next('a'));
  store.reset('a');
  const replay = store.start('a');
  assert.equal(replay.room.game.match, 1);
  assert.equal(Object.values(replay.room.game.scores).reduce((a,b) => a+b,0), 0);
});

test('limites des réglages validées côté serveur', () => {
  const store = createRoomStore();
  for (const phaseSeconds of [10,20,30,40,60]) assert.equal(store.create(String(phaseSeconds), 'Alice', { phaseSeconds }).room.settings.phaseSeconds, phaseSeconds);
  for (const timing of [{phaseSeconds: 300}, {phaseSeconds: 15}, {phaseSeconds: 0}, {phaseSeconds: 'unlimited'}, {matchCount: 6}, {matchCount: 0}, {maxTurns: 6}, {maxTurns: 0}, {maxTurns: 1.5}]) {
...
test('temps illimité : aucune expiration, passage après tous les indices et votes', () => {
  const store = createRoomStore();
  const { room } = store.create('a', 'Alice', { phaseSeconds: null, maxTurns: 2 });
  store.join('b', 'Bob', room.code); store.join('c', 'Charlie', room.code);
  const started = store.start('a');
  ...
  const order = started.room.game.clueOrder.map(id => ['a','b','c'][started.room.players.findIndex(p => p.id === id)]);
  for (const id of order.slice(0,2)) assert.equal(store.action(id, 'clue', { ...payload, text: 'Indice' }).game.phase, 'clues');
  const voting = store.action(order[2],'clue',{ ...payload, text:'Indice' });
  assert.equal(voting.game.phase,'vote');
  assert.equal(voting.game.deadline,null);
  assert.deepEqual(store.tick(Number.MAX_SAFE_INTEGER),[]);
  const ids = started.room.players.map(p => p.id);
  for (const [i,id] of ['a','b','c'].entries()) {
    const result = store.action(id,'vote',{ ...payload,targetId:ids[(i+1)%3] });
    if (i === 2) {
      assert.equal(result.game.turn,2);
...
test('indices séquentiels : refus hors tour, passage après délai, ordre conservé sans éliminés', () => {
  ...
  assert.throws(() => act(s, second, 'clue', { turn:1,text:'Trop tôt' },1), /pas à vous/);
  advance(s,10000);
  assert.equal(publicState(s).currentPlayerId,second);
  assert.equal(s.phase,'clues');
  assert.equal(s.deadline,20000);
  assert.throws(() => act(s,first,'clue',{turn:1,text:'Trop tard'},10001), /pas à vous/);
  for (const id of s.clueOrder.slice(1)) act(s,id,'clue',{turn:1,text:'Indice'},10001);
  assert.equal(s.phase,'vote');
